import React, { useState, useEffect, useRef, useMemo } from 'react'
        ...
        function App() {
            ...
            // 🆕 V15.02: provenance badge for the word modal. Four states; the two AI ones reuse the
            // list dot's colours, Manual and Unknown stay deliberately neutral so they do not compete
            // with them. NULL still means "unknown origin" — old records are never migrated.
            const SourceBadge = ({ record }) => {
                if (!record) return null;
                const source = record.ai_source;
                const hasContent = !!(record.synonyms?.trim() || record.context?.trim() || record.family?.trim());

                let label, title, cls;
                if (source === 'gemini') {
                    label = '✨ Gemini';
                    title = 'Content generated by Gemini';
                    cls = 'bg-blue-500/15 text-blue-300 border-blue-400/50';
                } else if (source === 'groq') {
                    label = '⚡ Groq';
                    title = 'Content generated by Groq';
                    cls = 'bg-orange-500/15 text-orange-300 border-orange-400/50';
                } else if (source === 'manual') {
                    label = '✍️ Manual';
                    title = 'Edited by you — no model wrote this version';
                    cls = 'bg-slate-600/30 text-slate-200 border-slate-400/40';
                } else if (hasContent) {
                    label = '❓ Unknown origin';
                    title = 'Has content but no recorded model: either you wrote it, or it predates provenance tracking. The two cannot be distinguished.';
                    cls = 'bg-slate-700/40 text-slate-400 border-slate-600/50';
                } else {
                    return null; // empty record — nothing has produced anything yet
                }

                return (
                    <span title={title} className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full border whitespace-nowrap cursor-help ${cls}`}>
                        {label}
                    </span>
                );
            };
...
Reply ONLY: {"usage":"...","alternative":"..."}`
            ...
            async function fetchWords(pageNum, isNewSearch = false) {
                ...
                try {
                    ...
                    // 🆕 V15.02: edited by hand. Distinct from UnknownOrigin, which stays NULL + has content
                    else if (emptyFilter === 'ManualEdited') query = query.eq('ai_source', 'manual');
...
Return ONLY the JSON object.`;
            ...
            // 🆕 V14.97: desktop only. The round-trip means copying into another AI in a separate tab
            // and pasting the reply back, which is not phone work, and three items crowded the header
            // corner on narrow screens. 1024px matches the app's own mobile/desktop line in styles.css.
            const REVIEW_BTN = 'hidden lg:inline-block px-2.5 py-1 rounded-lg text-[10px] lg:text-xs font-black uppercase whitespace-nowrap border transition-colors bg-teal-600/20 text-teal-300 border-teal-500/40 hover:bg-teal-600/30 disabled:bg-slate-800/60 disabled:text-slate-600 disabled:border-transparent';
            ...
            const handleMagicFill = async (word, targetFields = null, wordId = null) => {
                ...
                try {
                    ...
                    if (targetFields) {
                        ...
                        // 🆕 V15.02: derived from the payload actually written, not from a hand-listed
                        // subset. It used to merge updateData (three fields) while writing seven, so
                        // usage_level, ai_source, previous_version and modified_at stayed stale in
                        // memory — the modal read the old usage_level and the list dot never appeared
                        // until a reload. Spreading the payload keeps the two in step by construction.
                        const updatedWord = { ...(currentData || words.find(w => w.id === targetId)), ...updateDataWithHistory };
...
Respond ONLY in this exact JSON format (no markdown, no backticks):
{
  ...
  "family": "${currentFamily}"
            ...
            async function handleSave(e) {
                ...
                // 🆕 V15.02: normalised BEFORE the change comparison below, so a value that merely
                // differs in formatting is not mistaken for a manual edit
                wordData.usage_level = normaliseUsageLevel(wordData.usage_level);

                // 🆕 V15.02: provenance on save.
                // This previously wrote NULL on ANY manual save, which wiped the model marker off a
                // record merely opened and confirmed. Now:
                //   Magic Fill ran in this modal → the model that answered (it is not a manual edit)
                //   a field actually changed     → 'manual'
                //   nothing changed              → leave the existing value untouched
                // Existing NULLs are never migrated: an untouched old record stays "unknown origin".
                const editableFields = ['vocabulary', 'synonyms', 'context', 'family', 'usage_level'];
                const baseline = originalEditData || editingWord;
                const somethingChanged = !editingWord || !baseline
                        ...
                        <div className="max-w-[1850px] mx-auto flex flex-col gap-4">
                            ...
                            <div className="flex flex-wrap items-center gap-2 lg:gap-3 justify-center lg:justify-start relative">
                                ...
                                <select value={emptyFilter} onChange={e => setEmptyFilter(e.target.value)} className="p-2 lg:p-2.5 rounded-xl text-xs font-bold uppercase bg-slate-800 text-red-400 flex-1 min-w-[85px] sm:flex-initial">
                                    ...
                                    {/* 🆕 V15.02: the point of this one is knowing what NOT to reprocess with AI */}
                                    <option
                                        value="ManualEdited"
                            ...
                            <div className="glass-card rounded-[2.5rem] w-full max-w-2xl flex flex-col max-h-[90vh] overflow-hidden">
                                <div className="flex justify-between items-center px-8 pt-8 pb-5 shrink-0 border-b border-white/5">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {/* 🆕 V15.02: where this record's content came from. Per record,
                                            not per field. Same colours as the list dot so the visual
                                            criterion stays consistent. */}
                                        {editingWord && <SourceBadge record={editingWord} />}
                                    </div>
                                </div>
